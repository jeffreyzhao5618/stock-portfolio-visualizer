    ...
    return new Promise((resolve, reject) => {
        ...
        axios({
          ...
          headers: {
            ...
            "x-rapidapi-key":
              "3c3c309a6amshec3bde770cc93ecp10a5cfjsn5068fba5792a",
            useQueryString: true
          },
          params: {
            region: "US",
        ...
        if(!funds.includes(data[i].fund)) {
            ...
            for(let j in fundTickers) {
                if(fundTickers[j].name == data[i].ticker) {
                    fundTickers[j].value += data[i].ticker == 'USD' ? Number(data[i].totalPrice) : 0;
        ...
        for(let i in squarifyObject) {
            for(let j in squarifyObject[i].children) {
                ...
                if(currTicker.name == 'USD') {
                    ...
                    currTicker.value = quotes[quoteIndex].regularMarketPrice * currTicker.quantity;
                    currTicker.marketPrice = quotes[quoteIndex].regularMarketPrice
                    squarifyObject[i].value += quotes[quoteIndex].regularMarketPrice * currTicker.quantity;
                    currTicker.percentChange = quotes[quoteIndex].regularMarketChangePercent;
                }
            }
        }
